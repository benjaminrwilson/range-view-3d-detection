...
def mat_to_quat(mat: Tensor) -> Tensor:
    ...
    """
    return C.rotation_matrix_to_quaternion(mat, order=C.QuaternionCoeffOrder.WXYZ)


@torch.jit.script
def quat_to_xyz(quat_wxyz: Tensor, singularity_value: float = PI / 2) -> Tensor:
...
def yaw_to_quat(yaw_rad: Tensor) -> Tensor:
    ...
    Returns:
        (...,4) scalar first quaternions (wxyz).
    """
    roll = torch.zeros_like(yaw_rad[:, -1:])
    pitch = torch.zeros_like(yaw_rad[:, -1:])
    qw, qx, qy, qz = quaternion_from_euler(roll, pitch, yaw_rad)
