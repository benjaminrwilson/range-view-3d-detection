...
from typing import Any, Callable, Final, Tuple, cast
...
def benchmark(
    ...
    for i, index in enumerate(tqdm(indices)):
        ...
        if log_outputs:
            ...
            log_id, timestamp_ns, _ = uuids.filter(pl.col("batch_index") == 0).row(0)
    ...
    metrics = metrics.with_columns(
        backbone=breakdown_averages["backbone"].item(),
        head=breakdown_averages["head"].item(),
        decoder=breakdown_averages["decoder"].item(),
        total=breakdown_averages["total"].item(),
        fps=fps,
    )
