...
def draw_on_bev(
    ...
    for i, (vertices, score) in enumerate(zip(vertices, scores)):
        vertices, _ = world_to_image(
            vertices, max_side, meter_per_px, mask_invalid=False
        )
...
def world_to_image(
    xyz: torch.Tensor, max_side: int, meter_per_px: int, mask_invalid: bool = True
) -> torch.Tensor:
